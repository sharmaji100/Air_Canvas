...
# Creating the trackbar for adjusting line width
cv2.createTrackbar("Line Width", "Color detectors", 2, 10, setValues)
...
while True:
    ...
    # Get line width from the trackbar
    line_width = cv2.getTrackbarPos("Line Width", "Color detectors")
        ...
        for j in range(len(points[i])):
            for k in range(1, len(points[i][j])):
                if points[i][j][k - 1] is None or points[i][j][k] is None:
                    continue
                cv2.line(frame, points[i][j][k - 1], points[i][j][k], colors[i], line_width)
                cv2.line(paintWindow, points[i][j][k - 1], points[i][j][k], colors[i], line_width)
